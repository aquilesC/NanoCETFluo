import json
import time

import h5py
import numpy as np
import zmq

from experimentor import Q_
from experimentor.core.meta import ExperimentorProcess


class DaqSaver(ExperimentorProcess):
    """ Saver for the data generated by the DAQ.
    """
    def __init__(self, file, max_memory, saving_event, url, topic='', metadata=None):
        super().__init__()
        self.file = file
        self.max_memory = max_memory
        self.saving_event = saving_event
        self.topic = topic
        self.url = url
        self.stop_keyword = "DAQSaverStop"
        if metadata is None:
            metadata = {}
        for key, value in metadata.items():
            if isinstance(value, Q_):
                metadata[key] = str(value)
        self.metadata = metadata
        self.start()

    def run(self) -> None:
        self.logger.info('Starting logger')
        context = zmq.Context()
        socket = context.socket(zmq.SUB)
        socket.connect(self.url)
        socket.setsockopt(zmq.SUBSCRIBE, self.topic.encode('utf-8'))
        meta = {}


        with h5py.File(self.file, "a") as f:
            g = f.create_group('data')
            i = 0
            j = 0
            first = True
            while not self.saving_event.is_set():

                event = socket.poll(0)
                if not event:
                    time.sleep(.005)
                    continue
                topic = socket.recv_string()
                metadata = socket.recv_json(flags=0)
                msg = socket.recv(flags=0, copy=True, track=False)
                if not metadata.get('numpy', False):
                    self.logger.info('Got stop keyword')
                    break

                buf = memoryview(msg)
                data = np.frombuffer(buf, dtype=metadata['dtype'])
                data = data.reshape(metadata['shape'], order="F").copy()

                # Using byte order F gives the proper shape, but it is camera-dependent
                # This works fine for Basler, but need to keep an eye for the future
                # TODO: standardize the byte-order for camera frames, are they always Fortran?

                if first:  # First time it runs, creates the dataset
                    x = data.shape[0]
                    allocate = int(self.max_memory / data.nbytes * 1024 * 1024)
                    self.logger.info(f'Allocation {allocate} frames in HDF5 file')
                    d = np.zeros((x, allocate), dtype=data.dtype)
                    dset = g.create_dataset('timelapse', (x, allocate), maxshape=(x, None),
                                            compression='gzip', compression_opts=1,
                                            dtype=data.dtype)  # The images are going to be stacked along the z-axis.
                    first = False
                    meta = {
                        'start': time.time(),
                        'allocate': allocate,
                    }
                    meta.update(self.metadata)
                    metadata = json.dumps(meta)
                    mdset = g.create_dataset('metadata', data=metadata.encode("utf-8", "ignore"))

                d[:, i] = data
                i += 1

                if i == allocate:
                    dset[:, j:j + allocate] = d
                    dset.resize((x, j + 2*allocate))
                    d = np.zeros((x, allocate), dtype=data.dtype)
                    i = 0
                    j += allocate

            if i != 0:
                self.logger.info(f'Saving last {i} frames')
                dset[:, j:j + i] = d[:, :i]

            meta.update({
                'end': time.time(),
                'frames': j+i,
                'allocate': allocate,
            })
            metadata = json.dumps(meta)
            mdset[()] = metadata.encode("utf-8", "ignore")
            self.logger.info(f'Saver finished, total acquired frames: {j+i}')

