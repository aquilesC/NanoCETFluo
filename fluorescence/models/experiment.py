...
from .basler import BaslerCamera as Camera
...
from .daq_saver import DaqSaver
...
class FluorescenceMeasurement(Experiment):
    ...
    def start_saving_daq_and_images(self):
        if self.saving:
            self.logger.warning("Saving process still running. self.saving is true")
        if (self.saving_process is not None or self.daq_saving_process is not None) and (self.saving_process.is_alive() or self.daq_saving_process.is_alive()):
            self.logger.warning('Saving process is alive. Please stop saving first')
            return

        self.camera_microscope.stop_continuous_reads()
        self.camera_microscope.stop_free_run()
        self.daq.stop_continuous_reads()

        self.saving = True
        base_filename_movie = self.config['info']['filename_movie']
        file_movie = self.get_filename(base_filename_movie)
        base_filename_daq = self.config['info']['filename_daq']
        file_daq = self.get_filename(base_filename_daq)
        self.saving_event.clear()
        self.saving_process = MovieSaver(
            file_movie,
            self.config['saving']['max_memory'],
        ...
        self.daq_saving_process = DaqSaver(
            file_daq,
            self.config['saving']['max_memory_daq'],
            self.saving_event,
            url=self.daq.new_data.url,
            topic='new_data',
            metadata=self.daq.config,
        )

        self.daq.continuous_reads()
        self.camera_microscope.start_free_run()
        self.camera_microscope.continuous_reads()

    def stop_saving_daq_and_images(self):
        self.stop_saving_images()
        self.daq.new_data.emit('stop')
        time.sleep(0.05)
        if self.daq_saving_process is not None and self.daq_saving_process.is_alive():
            self.logger.warning('DAQ saving process still alive.')
            time.sleep(.1)
        self.saving = False
